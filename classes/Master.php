...
Class Master extends DBConnection {
	...
	function save_dtopic(){
		...
		$check = $this->conn->query("SELECT * FROM `dtopic` where `name` = '{$name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
		...
		if(empty($id)){
			$sql = "INSERT INTO `dtopic` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `dtopic` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
	...
	function delete_dtopic(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `dtopic` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Topic successfully deleted.");
		}else{
	...
	function upload_files_donation(){
		extract($_POST);
		$data = "";
		if(empty($upload_code)){
			while(true){
				$code = $this->generate_string($this->permitted_chars);
				$chk = $this->conn->query("SELECT * FROM `upload` where dir_code ='{$code}' ")->num_rows;
				if($chk <= 0){
					$upload_code = $code;
					$resp['upload_code'] =$upload_code;
					break;
		...
		if(!is_dir(base_app.'upload/blog_uploads/'.$upload_code))
			mkdir(base_app.'upload/blog_uploads/'.$upload_code);
		$dir = 'upload/blog_uploads/'.$upload_code.'/';
		$images = array();
		for($i = 0;$i < count($_FILES['img']['tmp_name']); $i++){
			if(!empty($_FILES['img']['tmp_name'][$i])){
				$fname = $dir.(time()).'_'.$_FILES['img']['name'][$i];
				...
				$move = move_uploaded_file($_FILES['img']['tmp_name'][$i],base_app.$fname);
				if($move){
					$this->conn->query("INSERT INTO `upload` (dir_code,user_id,file_path)VALUES('{$upload_code}','{$this->settings->userdata('id')}','{$fname}')");
					$this->capture_err();
					$images[] = $fname;
				}
			}
	...
	function save_donation(){
		...
		if(empty($id)){
			$data .= ", `author_id`='{$this->settings->userdata('id')}' ";
			$sql = "INSERT INTO `donation` set {$data} ";
			$save = $this->conn->query($sql);
			$id= $this->conn->insert_id;
		}else{
			$sql = "UPDATE `donation` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"New post successfully saved.");
			else
				$this->settings->set_flashdata('success',"post successfully updated.");
			$id = empty($id) ? $this->conn->insert_id : $id;
			$dir = 'upload/blog_uploads/banners/';
			if(!is_dir(base_app.$dir))
				mkdir(base_app.$dir);
			if(isset($_FILES['banner_image'])){
				if(!empty($_FILES['banner_image']['tmp_name'])){
					...
					$move =  move_uploaded_file($_FILES['banner_image']['tmp_name'],base_app.$fname);
					if($move){
						$this->conn->query("UPDATE `donation` set `banner_path` = '{$fname}' where id = '{$id}' ");
					}
				}
			}
			if(!isset($fnme))
				$fname = $this->conn->query("SELECT banner_path FROM `donation` where id = '{$id}' ")->fetch_array()['banner_path'];
			$date_created = $this->conn->query("SELECT date_created FROM `donation` where id = '{$id}' ")->fetch_array()['date_created'];
			...
			if(!is_file(base_app.$blog_url))
				...
				$contents .= ($content);
				$contents .= "\n";
				$contents .= "<?php include(base_app.'inc/body_block_ends.php') ?>\n";
				file_put_contents(base_app.$blog_url, html_entity_decode($contents));
	...
	function delete_donation(){
		extract($_POST);
		$qry = $this->conn->query("SELECT * FROM `donation` where id = '{$id}'");
		foreach($qry->fetch_array() as $k =>  $v){
			$meta[$k] = $v;
		}
		$del = $this->conn->query("DELETE FROM `donation` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			if(is_file(base_app.$meta['blog_url']))
				unlink((base_app.$meta['blog_url']));
	...
	function save_enrollment(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Donation was successfully submitted";
			else
				$resp['msg'] = "Donation was updated successfully.";
		}else{
			$resp['status'] = 'failed';
	...
	function delete_enrollment(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Donation Records has deleted successfully.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_video(){
		...
		if(empty($id)){
			$sql = "INSERT INTO `videos` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `videos` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			if(empty($id))
				$this->settings->set_flashdata('success',"New Video successfully saved.");
			else
				$this->settings->set_flashdata('success',"Video successfully updated.");
			$resp['status'] = 'success';
			$id = empty($id) ? $this->conn->insert_id : $id;
		}
		return json_encode($resp);
	...
	function delete_video(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `videos` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Event successfully deleted.");
	...
	function save_live(){
		...
		if(empty($id)){
			$sql = "INSERT INTO `live` SET {$data} ON DUPLICATE KEY UPDATE live_url = VALUES(live_url), date_created = VALUES(date_created)";

			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `live` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			if(empty($id))
				$this->settings->set_flashdata('success',"New live link successfully saved.");
			else
				$this->settings->set_flashdata('success',"Live link successfully updated.");
			$resp['status'] = 'success';
			$id = empty($id) ? $this->conn->insert_id : $id;
		}
		return json_encode($resp);
...
switch ($action) {
	...
	break;
	case 'save_dtopic':
		echo $Master->save_dtopic();
	break;
	case 'delete_dtopic':
		echo $Master->delete_dtopic();
	break;
	...
	break;
	case 'upload_files_donation':
		echo $Master->upload_files_donation();
	break;
	case 'save_blog':
