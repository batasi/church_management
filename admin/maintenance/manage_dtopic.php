...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `dtopic` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="card card-outline card-info">
	...
	<div class="card-footer">
		...
		<a class="btn btn-flat btn-default" href="?page=maintenance/dtopics">Cancel</a>
	</div>
</div>
<script>
	...
	$(document).ready(function(){
		$('#topic-form').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f=save_dtopic",
				...
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
						location.href = "./?page=maintenance/dtopics";
